...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  input_to_hid_decay_grad = model.input_to_hid * wd_coefficient;
  hid_to_class_decay_grad = model.hid_to_class * wd_coefficient;
  ...
  last_layer_delta = class_prob - data.targets; % <number of classes> by <number of data cases>
  hid_layer_delta = logistic(hid_input) .* (1 - logistic(hid_input)) .* (model.hid_to_class.' * last_layer_delta); % <number of hidden units> by <number of data cases>
  hid_to_clas_class_grad = last_layer_delta * hid_output.' / size(data.inputs, 2); % <number of classes> by <number of hidden>
  input_to_hid_class_grad = hid_layer_delta * data.inputs.' / size(data.inputs, 2);  % <number of hid> by <number of inputs> Because the error is average over the data set, we must divide the gradient by the data set length
  ...
  ret.input_to_hid = input_to_hid_decay_grad + input_to_hid_class_grad;
  ret.hid_to_class = hid_to_class_decay_grad + hid_to_clas_class_grad;
end
